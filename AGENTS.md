...
This repo is a fork of `badlogic/pi-mono`, customized by `WillChangeThisLater/pi`.

### Sync Upstream Changes

Use the helper script to pull latest from the badlogic upstream:

```bash
# Update main only
./scripts/sync-upstream.sh

# Update main + rebase current feature branch
./scripts/sync-upstream.sh --rebase-branch
```

This stashes dirty worktree, fast-forwards `main` from `upstream`, pushes to `origin`, and optionally rebases your current branch on the new `main`.
